...
impl NoteTimeMap {
    ...
    pub fn push(&mut self, note: &Note) {
        let mut insert = |shift: f64| {
            ...
            let now_time =
                last_point.2 + last_point.1 * (note.note_time.measure_notation - last_point.0);
            let now_mpm = last_point.1 * shift;
            self.critical_points
                .push((note.note_time.measure_notation, now_mpm, now_time));
        };

        match note.note_action {
            NoteAction::BPMChange { bpm } => {
                //println!("{:?} {:?}",self.bpm/bpm,note);
                insert(self.bpm/bpm);
                self.bpm = bpm;
            }
